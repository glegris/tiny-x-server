...
public class DashOffset implements GraphicsAttributeHandler {
	...
	public void read(
		...
		final int dashOffset = inputStream.readUnsignedShort();
		graphicsContext.setDashOffset(dashOffset);
	}

	@Override
	public void write(final XOutputStream outputStream, final GraphicsContext graphicsContext) throws IOException {
		outputStream.writeInt(graphicsContext.getDashOffset());
	}

	@Override
	public void copy(final GraphicsContext source, final GraphicsContext destination) {
		destination.setDashOffset(source.getDashOffset());
	}
}
